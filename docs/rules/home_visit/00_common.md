...
- 往診管理：往診の管理者のみが編集・修正できる権限。管理者以外は見れない・触れない。
- 往診事務：スタッフや管理者が入力した内容を、事務所側が確認するだけの場所。往診管理者も見れるが、ここは基本的に閲覧専用。唯一の例外は往診売上で、その権限（isAccounting）を持つ人だけ入力・閲覧できる。
- 名前が似ていても意味は全く違うので、混同しない。
