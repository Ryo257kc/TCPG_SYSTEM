...
スタッフ側の区分プルダウンは `一般保険／介護保険／社会保険／地震保険／年金保険` に加えて、
`小規模企業共済（機構）／企業型年金（DC）／個人型年金（iDeCo）` を選べる
（2026-08-21追加。管理側`groupHokenRows()`のキーワード判定「機構」「企業型」「個人型」に
一致する文言にしてある。文言を変える場合はこの判定ロジックとセットで確認すること）。

`心身障害者扶養共済`はキーワード判定自体は存在する（`groupHokenRows()`の`shinshin`）が、
PDF側に書き込み処理が無い（`writeHokenSmallBusinessRow`がkikou/kigyo/kojinのみ呼ばれている）。
極めて稀なケースのため未対応のまま。対象者が出たら両方セットで実装すること。
