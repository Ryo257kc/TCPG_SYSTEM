...
| 合計所得金額 | 令和6年分以前 | 令和7年分 | 令和8・9年分 | 令和10年分以後 |
|---|---|---|---|---|
| 132万円以下 | 48万円 | 95万円 | 99万円 | 99万円 |
...
`basicDeductionAmount()` は `targetYear` で4系統（〜2024年分／2025年分／2026・2027年分／2028年分〜）
に分岐する。

> **令和8・9年分の数字は令和8年度税制改正であらためて引き上げられたもの（2026-09-12修正）**：
> 上表の「令和8・9年分」は国税庁「令和8年分 年末調整のしかた」3ページ
> （https://www.nta.go.jp/publication/pamph/gensen/nencho2026/01.htm ）で確認した最新の数字。
> それ以前（2026年8月時点）は、令和8年分は撤回された旧予定の数字（132万円超2,350万円以下が
> 一律58万円）のまま実装されていた。年調是正で過去年分を再計算しても数字が動かないよう、
> 「令和8・9年分（2026年・2027年）」と「令和10年分以後（2028年〜）」を別ブランチにしてある。
...
## 給与所得控除の最低保障額の特例（令和8・9年分限定）

令和8年度税制改正で、給与所得控除の最低保障額が65万円→74万円に引き上げられた。加えて、
収入69万1,000円以上220万円未満は上記引き上げにかかわらず専用の特例表（下限額は`salaryIncomeAfterDeduction()`
のコメント参照）を使う。220万円以上は令和7年分までと同じ計算式（変更なしと明記されている）。

国税庁「令和8年分 年末調整のしかた」3ページで確認済み（2026-09-12）。

> **令和10年（2028年）分以後は要確認**：同じページに令和10年分以後の記載
> （「その収入金額×30%+8万円（69万円未満となる場合は69万円）」）があるが、既存の65万円保証の
> 式との関係が未確認のため、`salaryIncomeAfterDeduction()`では令和10年分をtargetYear>=2025の
> 65万円保証の式にフォールバックさせたまま分岐を追加していない。令和10年分が近づいたら
> 公式資料で確認してから分岐を追加すること。

## 扶養親族等の所得要件の引き上げ（令和8年分以後、恒久的な変更）

令和8年度税制改正で、扶養親族等の合計所得金額の要件が軒並み4万円引き上げられた
（国税庁「令和8年分 年末調整のしかた」17〜18ページで確認済み、2026-09-12）。

| 区分 | 改正前 | 改正後（令和8年分以後） |
|---|---|---|
| 扶養親族・同一生計配偶者・ひとり親の生計を一にする子 | 58万円以下 | 62万円以下 |
| 特定親族特別控除の対象（特定親族） | 58万円超123万円以下 | 62万円超123万円以下（上限は変更なし） |
| 配偶者特別控除の対象となる配偶者 | 58万円超133万円以下 | 62万円超133万円以下（上限は変更なし） |
| 勤労学生 | 85万円以下 | 89万円以下（未実装、対象者が出たら追加） |

配偶者特別控除・特定親族特別控除とも、**しきい値以外の内訳の金額表（9段階の金額そのもの）は
変更なし**。`spousalDeductionAmounts()`／`tokuteiShinzokuTokubetsuKoujoAmount()`とも、
入口の判定（58万円→62万円）だけを`targetYear>=2026`で切り替えている。

年度を跨いだ「令和8・9年分限定」のような記載がこのページには無いため、恒久的な変更という
理解で実装した（基礎控除・給与所得控除の2年限定の特例とは違う）。令和10年分以後で
また変わることが分かれば、この理解を修正すること。

## 一般生命保険料控除の特例（23歳未満の扶養親族を有する場合、令和8・9年分限定）

令和8年度税制改正で新設（国税庁「令和8年分 年末調整のしかた」17ページで確認済み、
2026-09-12）。対象は**新生命保険料に係る一般生命保険料控除のみ**（介護医療保険料・個人年金
保険料は対象外、旧生命保険料も対象外＝計算式Ⅱのまま）。
...
新旧合算後の一般生命保険料控除の上限も4万円→6万円に変わる（一般＋介護医療＋個人年金の
合計適用限度額12万円は変更なし）。

計算は`YearEndAdjustmentV2Controller::hokenNewLifeInsuranceCalcUnder23Special()`（新設）で行い、
`writeHokenLifeInsuranceRow()`のgeneral区分（一般生命保険料）でのみ使う。対象かどうかの判定は
`YearEndCalculationService::hasDependentUnder23()`（配偶者を除く扶養親族で年齢23歳未満が
1人でもいるか、`deduction_target=1`の行のみ）。`mx_nen_tyo.seimei_fee_kou`（生命保険料控除額の
最終合計）自体は自動計算されず手入力保存のままだが、この計算式Ⅰ相当の中間値はPDF上に
印字されるため、正しい式を使う必要がある。
...
**年度分岐（重要）**： 58万円のしきい値・特定親族特別控除とも令和7年（2025年）分からの制度。
`spousalDeductionAmounts()`等の他の年度依存ロジックと同じく、`targetYear < 2025`では新ルールを
適用しない（年調是正で過去年分を再計算しても古い年の数字が動かないようにするため）。
2025年より前の正しいしきい値（改正前は合計所得48万円以下だったはず）は未確認・未実装で、
`targetYear < 2025`の間は旧仕様のまま（収入判定なし、年齢のみでtoku_fuに加算）にしている。
