...
<div class="">
    <div class="daily-head">
        ...
        <div class="daily-statuses">
            ...
            <button
                class="btn"
                type="submit"
                form="{{ $confirmFormId }}-cancel">承認解除</button>
            <form id="{{ $confirmFormId }}-cancel" method="post" action="{{ route('admin.attendance.cancel-confirm') }}">
                @csrf
                <input type="hidden" name="month" value="{{ $selectedMonth }}">
                <input type="hidden" name="company_id" value="{{ $selectedCompanyId }}">
                <input type="hidden" name="staff_id" value="{{ $detailStaff['staff_id'] }}">
            </form>
            @endif
        </div>
    </div>
    ...
    <div class="daily-summary">
        <div class="daily-summary-group">
            ...
            <div class="daily-summary-row">
                ...
                <div class="daily-summary-value">{{ number_format((float) ($dailySummary['paid_leave_days'] ?? 0), 1) }}</div>
            </div>
        </div>
        <div class="daily-summary-group">
    ...
    <div class="daily-wrap">
        <table class="data-table f_size12 center word">
            <colgroup>
            ...
            <thead>
                <tr class="center">
                    ...
                    <th class="f-sen daily-group-start daily-work-store-col" rowspan="2">勤務店舗</th>
                    <th class="f-sen" colspan="3">備考</th>
                ...
                <tr>
                    ...
                    <th class="">勤怠</th>
                    <th colspan="2" class="f-sen">差戻し</th>
                </tr>
            </thead>
            <tbody>
                ...
                <tr @class(['attendance-daily-row', 'row-holiday'=> ($row['is_holiday'] ?? '0') === '1'])>
                    ...
                    <td>
                        <span class="daily-view">{{ $row['holiday_category'] }}</span>
                        <select class="daily-edit"
                            name="holiday_category"
                            form="{{ $formId }}"
                            data-original="{{ $row['holiday_category'] }}">
                            <option value=""></option>
                            <option value="平日" @selected($row['holiday_category']==='平日' )>平日</option>
                            <option value="半日" @selected($row['holiday_category']==='半日' )>半日</option>
                    ...
                    <td>
                        ...
                        <select class="daily-edit daily-edit-input-category" name="attendance_category" form="{{ $formId }}" data-original="{{ $row['attendance_category'] }}">
                            <option value="">--</option>
                    ...
                    <td>
                        ...
                        <input class="daily-edit daily-edit-input daily-edit-input-num" type="text" step="0.01" name="category_time" value="{{ $row['category_time'] }}" form="{{ $formId }}" data-original="{{ $row['category_time'] }}">
                    </td>
                    <td class="f-sen">
                        <span class="daily-view">{{ $row['paid_leave_used'] }}</span>
                        <input class="daily-edit daily-edit-input daily-edit-input-num" type="text" step="0.01" name="paid_leave_used" value="{{ $row['paid_leave_used'] }}" form="{{ $formId }}" data-original="{{ $row['paid_leave_used'] }}">
                    </td>
                    ...
                    <td>
                        ...
                        <input class="daily-edit daily-edit-input daily-edit-input-num" type="text" step="0.01" name="overtime" value="{{ $row['overtime'] }}" form="{{ $formId }}" data-original="{{ $row['overtime'] }}">
                    </td>
                    <td class="f-sen">
                        <span class="daily-view">{{ $row['night_overtime'] }}</span>
                        <input class="daily-edit daily-edit-input daily-edit-input-num" type="text" step="0.01" name="night_overtime" value="{{ $row['night_overtime'] }}" form="{{ $formId }}" data-original="{{ $row['night_overtime'] }}">
                    </td>
                    <td class="f-sen daily-group-start daily-work-store-col">
                    ...
                    <td class="daily-note-first daily-note-cell" title="{{ $row['timecard_note'] }}" ondblclick="openDailyNoteEditor(this)">
                    ...
                    <td>
                        {{-- 表示用 --}}
                        <span class="daily-view">
                            <input type="checkbox" disabled {{ !empty($row['is_returned']) ? 'checked' : '' }}>
                        </span>

                        {{-- 編集用 --}}
                        <input type="hidden" name="is_returned" value="0" form="{{ $formId }}">
                        <input class="daily-edit"
                            type="checkbox"
                            name="is_returned"
                            value="1"
                            form="{{ $formId }}"
                            {{ !empty($row['is_returned']) ? 'checked' : '' }}>
                    </td>
                    <td class="daily-note-cell f-sen" title="{{ $row['return_note'] }}" ondblclick="openDailyNoteEditor(this)">
