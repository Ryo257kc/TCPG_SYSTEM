...
(function(){
  ...
  if (recalculateBtn) {
    recalculateBtn.addEventListener('click', function(){
      ...
      .then(function(res){ return res.json(); })
      .then(function(json){
        if (!json || json.ok !== true) throw new Error('recalculate failed');
        location.reload();
      })
      .catch(function(){
        alert('賞与データの再計算に失敗しました。');
      })
      .finally(function(){
  ...
  var loadCandidates = function(){
    ...
    fetch(createCandidatesUrl + query, { headers: { 'Accept': 'application/json' }})
      .then(function(res){ return res.json(); })
      .then(function(json){
  ...
  if (selectAllBtn) selectAllBtn.addEventListener('click', function(){ document.querySelectorAll('.bonus-create-check').forEach(function(check){ check.checked = true; }); });
  if (clearBtn) clearBtn.addEventListener('click', function(){ document.querySelectorAll('.bonus-create-check').forEach(function(check){ check.checked = false; }); });

  if (createSubmitBtn) {
    createSubmitBtn.addEventListener('click', function(){
      var ids = Array.prototype.slice.call(document.querySelectorAll('.bonus-create-check:checked')).map(function(check){ return check.value; });
      var selectedPayment = createPaymentDate ? createPaymentDate.value : '';
      if (ids.length === 0) { alert('スタッフを選択してください。'); return; }
      if (!selectedPayment) { alert('支給日を選択してください。'); return; }
      ...
      .finally(function(){ createSubmitBtn.disabled = false; });
    });
  }
  ...
  if (deleteSubmitBtn) {
    deleteSubmitBtn.addEventListener('click', function(){
      ...
      .finally(function(){ deleteSubmitBtn.disabled = false; });
    });
  }
})();
</script>
