...
<head>
    ...
    <link rel="stylesheet" href="{{ asset('css/staff_portal/insurers.css') }}">
</head>
<body>
<main class="container">
    ...
    @php
        $oldOriginalInsurerNumber = trim((string) old('original_insurer_number', ''));
    ...
    <section
        class="panel content-panel sales-panel"
        data-insurer-save-url="{{ route('office.insurers.save') }}"
        data-insurer-delete-url="{{ route('office.insurers.delete') }}"
        data-insurer-csrf="{{ csrf_token() }}"
    >
        <div class="content-head">
            <h1 class="content-title">保険者一覧</h1>
        </div>
        <form method="get" action="{{ route('office.insurers') }}" class="office-toolbar">
            <div class="office-toolbar-group">
                <label for="insurer-search">保険者検索</label>
                <input id="insurer-search" name="search" type="text" value="{{ $search ?? '' }}" class="insurer-inline-input">
        ...
        <div class="insurers-tab-row">
            <a href="{{ route('office.insurers', ['search' => ($search ?? ''), 'deposit_name_display' => ($depositNameFilter ?? 'all'), 'tab' => 'all']) }}" class="btn {{ ($activeTab ?? 'all') === 'all' ? 'btn-primary' : '' }}">全て</a>
            <a href="{{ route('office.insurers', ['search' => ($search ?? ''), 'deposit_name_display' => ($depositNameFilter ?? 'all'), 'tab' => 'general']) }}" class="btn {{ ($activeTab ?? 'all') === 'general' ? 'btn-primary' : '' }}">一般</a>
            <a href="{{ route('office.insurers', ['search' => ($search ?? ''), 'deposit_name_display' => ($depositNameFilter ?? 'all'), 'tab' => 'elderly']) }}" class="btn {{ ($activeTab ?? 'all') === 'elderly' ? 'btn-primary' : '' }}">後期高齢</a>
            <a href="{{ route('office.insurers', ['search' => ($search ?? ''), 'deposit_name_display' => ($depositNameFilter ?? 'all'), 'tab' => 'medical_support']) }}" class="btn {{ ($activeTab ?? 'all') === 'medical_support' ? 'btn-primary' : '' }}">医療助成</a>
            <a href="{{ route('office.insurers', ['search' => ($search ?? ''), 'deposit_name_display' => ($depositNameFilter ?? 'all'), 'tab' => 'incomplete']) }}" class="btn {{ ($activeTab ?? 'all') === 'incomplete' ? 'btn-primary' : '' }}">未入力項目</a>
            <button type="button" class="btn insurers-tab-add-btn" data-insurer-add>新規追加</button>
        </div>
        @if (session('errorMessage'))
            <div class="insurers-alert insurers-alert-error">{{ session('errorMessage') }}</div>
        @endif
        @if ($errors->any())
            <div class="insurers-alert insurers-alert-error">{{ $errors->first() }}</div>
        @endif
        @if (session('successMessage'))
            <div class="insurers-alert insurers-alert-success">{{ session('successMessage') }}</div>
        @endif
        ...
        <div class="table-wrap">
            <table class="data-table">
                ...
                <tbody>
                    @if ($isNewErrorRow)
                        <tr data-insurer-row data-is-new="1" data-is-editing="1" data-original-insurer-number="">
                            <td>
                                <span data-insurer-display>{{ old('insurer_number', '') }}</span>
                                <input type="text" value="{{ old('insurer_number', '') }}" class="insurer-inline-input" data-insurer-input hidden>
                            </td>
                            <td class="insurers-name-cell">
                                <span data-insurer-display>{{ old('insurer_name', '') }}</span>
                                <input type="text" value="{{ old('insurer_name', '') }}" class="insurer-inline-input" data-insurer-input hidden>
                            </td>
                            <td>
                                <span data-insurer-display>{{ old('scheduled_payment_name', '') }}</span>
                                <input type="text" value="{{ old('scheduled_payment_name', '') }}" class="insurer-inline-input" data-insurer-input hidden>
                            </td>
                            <td>
                                <span data-insurer-display>{{ old('scheduled_payment_name_2', '') }}</span>
                                <input type="text" value="{{ old('scheduled_payment_name_2', '') }}" class="insurer-inline-input" data-insurer-input hidden>
                            </td>
                            <td>
                                <span data-insurer-display>{{ old('insurance_type', '') }}</span>
                            ...
                            <td>
                                <span data-insurer-display>{{ old('subsidy_type', '') }}</span>
                                <select class="insurer-inline-input" data-insurer-input hidden>
                                    <option value=""></option>
                    ...
                    @endif
                    @forelse (($insurerRows ?? []) as $row)
                        @php
                            $isEditingRow = $hasOldInsurerInput && $oldOriginalInsurerNumber !== '' && $oldOriginalInsurerNumber === $row['insurer_number'];
                            $rowInsurerNumber = $isEditingRow ? trim((string) old('insurer_number', $row['insurer_number'])) : $row['insurer_number'];
                            $rowInsurerName = $isEditingRow ? trim((string) old('insurer_name', $row['insurer_name'])) : $row['insurer_name'];
                            $rowPaymentName = $isEditingRow ? trim((string) old('scheduled_payment_name', $row['scheduled_payment_name'])) : $row['scheduled_payment_name'];
                            $rowPaymentName2 = $isEditingRow ? trim((string) old('scheduled_payment_name_2', $row['scheduled_payment_name_2'])) : $row['scheduled_payment_name_2'];
                            $rowInsuranceType = $isEditingRow ? trim((string) old('insurance_type', $row['insurance_type'])) : $row['insurance_type'];
                            $rowSubsidyType = $isEditingRow ? trim((string) old('subsidy_type', $row['subsidy_type'])) : $row['subsidy_type'];
                        @endphp
                        <tr data-insurer-row data-original-insurer-number="{{ $row['insurer_number'] }}" @if($isEditingRow) data-is-editing="1" @endif>
                            ...
                            <td>
                                ...
                                <input type="text" value="{{ $rowPaymentName }}" class="insurer-inline-input" data-insurer-input hidden>
                            </td>
                            <td>
                                <span data-insurer-display>{{ $rowPaymentName2 }}</span>
                                <input type="text" value="{{ $rowPaymentName2 }}" class="insurer-inline-input" data-insurer-input hidden>
                            </td>
                            <td>
                    ...
                    @endforelse
                    <tr data-insurer-template data-original-insurer-number="" hidden>
                        <td>
                            <span data-insurer-display></span>
                            <input type="text" value="" class="insurer-inline-input" data-insurer-input hidden>
                        </td>
                        <td class="insurers-name-cell">
                            <span data-insurer-display></span>
                            <input type="text" value="" class="insurer-inline-input" data-insurer-input hidden>
...
@include('staff_portal.office.insurers.page_script')
