...
<body>
    <main class="container">
        ...
        <section
            class="panel content-panel receipt-entry-panel staff-viewport-panel"
            ...
            <form id="receipt-monthly-unlock-form" method="post" action="{{ route('office.receipt.entry.monthly_unlock') }}">
                @csrf
                <input type="hidden" name="target_month" value="{{ $targetMonth }}">
                <input type="hidden" name="store_name" value="{{ $selectedStore }}">
            </form>
            <form id="insurance-csv-import-form" method="POST" action="{{ route('office.receipt.entry.import') }}" enctype="multipart/form-data">
                @csrf
                <input type="hidden" id="insurance-csv-sejutsu-month" name="sejutsu_month" value="{{ $targetMonth ?? '' }}">
                <input type="hidden" id="insurance-csv-store-name" name="store_name" value="{{ $selectedStore ?? '' }}">
            </form>

            <div class="toolbar receipt-entry-main-toolbar">
                <form method="get" action="{{ route('office.receipt.entry') }}" class="toolbar-group receipt-entry-filter-form">
                    ...
                    <div>
                        <input type="file"
                            id="insurance-csv-file"
                            name="csv_file"
                            form="insurance-csv-import-form"
                            accept=".csv,text/csv">
                        <div class="receipt-import-status" id="insurance-csv-import-status" hidden>CSVを取込中です。画面を閉じずにお待ちください。</div>
                    </div>
                    @endunless
                </form>
                ...
                @else
                <button type="submit" class="btn apply" form="receipt-monthly-close-form" onclick="return confirm('{{ $targetMonth }}月分の月次締め処理をします。該当月の編集はできなくなります。');">レセ月次処理</button>
                @endif

                @if($is_admin)
                <button type="submit" class="btn master" form="receipt-monthly-unlock-form" onclick="return confirm('{{ $targetMonth }}月分のレセ月次処理を解除します。該当月の編集ができるようになります。');">レセ月次解除</button>
                @endif
            </div>

            <div class="table-wrap receipt-entry-list-wrap staff-viewport-list-wrap">
    ...
    @include('staff_portal.office.receipt.entry.page_script')
