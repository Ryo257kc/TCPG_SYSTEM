...
<body>
    <main class="container">
        ...
        <section class="panel content-panel staff-viewport-panel">
            <div class="content-head">
                <h2 class="content-title">基本シフト変更</h2>
            </div>
            ...
            <form method="post" action="{{ route('admin.basic-shift.update', ['shiftNo' => $shiftNo]) }}">
                ...
                <table class="data-table">
                    ...
                    <tr>
                        <th>始業</th>
                        <td><input type="time" name="シフト始業" step="900" value="{{ $shiftStart }}"></td>
                    </tr>
                    <tr>
                        <th>退出</th>
                        <td><input type="time" name="シフト退出" step="900" value="{{ $shiftExit }}"></td>
                    </tr>
                    <tr>
                        <th>入出</th>
                        <td><input type="time" name="シフト入出" step="900" value="{{ $shiftInOut }}"></td>
                    </tr>
                    <tr>
                        <th>終業</th>
                        <td><input type="time" name="シフト終業" step="900" value="{{ $shiftEnd }}"></td>
                    </tr>
                    <tr>
                        <th>勤務店舗</th>
                        <td>
    ...
    <script>
        ...
        (function() {
            ...
            var timeInputs = ids.map(function(name) {
            ...
            function syncRequired() {
                var hasAnyTime = timeInputs.some(function(el) {
                    return !!el.value;
                });
