@if (empty($rawRows) || count($rawRows) === 0)
    <div class="payroll-sheet-empty">当月の給与明細はありません。</div>
@else
    @php
        $first = $rawRows[0];
    ...
    <div class="payroll-sheet-wrap">
        <div class="payroll-sheet-title"><h2>{{ $sikyuMonth }} 給与明細書</h2></div>
        ...
        @foreach ($rawRows as $row)
            @php
                ...
                $otherTotal = (float)($row['adjustment_year_end'] ?? 0) + (float)($row['cost_liquidation'] ?? 0) + (float)($row['company_advance_cost'] ?? 0);
                $transfer = (float)($row['supply_sum'] ?? 0) - (float)($row['deduction_sum'] ?? 0) - (float)($row['transfer_balance'] ?? 0);
            ...
            <div class="payroll-sheet-table-wrap">
                <table class="payroll-sheet-table">
                    ...
                    <tr class="payroll-sheet-sum-row">
                        <td colspan="2"></td>
                        <td>支給合計</td><td>{{ $showYen($payTotal) }}</td>
                        <td>控除合計</td><td>{{ $showYen($deductTotal) }}</td>
                        <td>その他合計</td><td>{{ $showYen($otherTotal) }}</td>
                    </tr>
                    </tbody>
                </table>
            </div>
