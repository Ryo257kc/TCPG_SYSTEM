...
- TCPGSYSTEM_DEV.dbo.mx_time_cards の 2026/08 を同期する
...
- スタッフID041（寺前千穂）は新システムを実運用中で、その勤怠データが正なので
  対象から除外する（氏名/staff_name欄には彼女の分だけ生のstaff_id「041」が
  そのまま入っている。2026-09-12、ユーザー指示）。

注意:
- 読み取り元: TCPGSYSTEM
- 書き込み先: TCPGSYSTEM_DEV
- まずは COMMIT の前に件数を確認すること
- 前回の同期参考: 2026_03_sync_mx_time_cards_from_tcpgsystem_202602.sql
*/
